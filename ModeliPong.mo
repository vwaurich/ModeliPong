...
package ModeliPong
  model Game_keyboard
    ...
    Game game(vel0=1)
              annotation (Placement(transformation(extent={{50,-20},{78,0}})));
    /*
  ...
  */
      Modelica_DeviceDrivers.Blocks.InputDevices.KeyboardKeyInput player1_up(
        keyCode="Q")
      annotation (Placement(transformation(extent={{-160,20},{-140,40}})));
    Modelica_DeviceDrivers.Blocks.InputDevices.KeyboardKeyInput player1_down(keyCode="A")
      annotation (Placement(transformation(extent={{-160,-6},{-140,14}})));
    Modelica_DeviceDrivers.Blocks.InputDevices.KeyboardKeyInput player2_up(
        keyCode="O")
      annotation (Placement(transformation(extent={{-160,-40},{-140,-20}})));
    Modelica_DeviceDrivers.Blocks.InputDevices.KeyboardKeyInput player2_down(
        keyCode="L")
    ...
    Modelica.Blocks.Math.BooleanToReal booleanToReal2(realTrue=1)
      annotation (Placement(transformation(extent={{-120,-40},{-100,-20}})));
    Modelica.Blocks.Math.BooleanToReal booleanToReal3(realTrue=-1)
    ...
    Modelica.Blocks.Math.Min min
      annotation (Placement(transformation(extent={{0,16},{10,26}})));
    ...
    Modelica.Blocks.Math.Max max2
      annotation (Placement(transformation(extent={{18,16},{30,26}})));
    Modelica.Blocks.Math.Min min1
      annotation (Placement(transformation(extent={{-2,-52},{8,-42}})));
    Modelica.Blocks.Math.Max max1
      annotation (Placement(transformation(extent={{18,-52},{30,-42}})));
  equation
    connect(player1_up.keyState, booleanToReal.u)
      annotation (Line(points={{-139,30},{-122,30}}, color={255,0,255}));
    connect(booleanToReal1.u, player1_down.keyState)
      annotation (Line(points={{-122,4},{-139,4}},          color={255,0,255}));
    connect(booleanToReal2.u, player2_up.keyState) annotation (Line(points={{-122,
            -30},{-139,-30}},            color={255,0,255}));
    connect(booleanToReal3.u, player2_down.keyState) annotation (Line(points={{-122,
            -56},{-139,-56}},            color={255,0,255}));
    connect(integrator.u, booleanToReal.y)
      annotation (Line(points={{-82,30},{-99,30}},  color={0,0,127}));
    connect(integrator1.u, booleanToReal1.y)
      annotation (Line(points={{-82,4},{-82,4},{-99,4}},  color={0,0,127}));
    connect(booleanToReal2.y, integrator2.u)
      annotation (Line(points={{-99,-30},{-82,-30}}, color={0,0,127}));
    connect(integrator3.u, booleanToReal3.y)
      annotation (Line(points={{-82,-56},{-99,-56}}, color={0,0,127}));
    connect(realExpression.y, min.u1) annotation (Line(points={{-19,56},{-4,56},{-4,
            24},{-1,24}}, color={0,0,127}));
    connect(add.u1, integrator.y) annotation (Line(points={{-44,24},{-54,24},{-54,
            30},{-59,30}}, color={0,0,127}));
    connect(add.u2, integrator1.y) annotation (Line(points={{-44,12},{-54,12},{-54,
            4},{-59,4}}, color={0,0,127}));
    connect(add1.u1, integrator2.y) annotation (Line(points={{-42,-38},{-56,-38},{
            -56,-30},{-59,-30}}, color={0,0,127}));
    connect(add1.u2, integrator3.y) annotation (Line(points={{-42,-50},{-56,-50},{
            -56,-56},{-59,-56}}, color={0,0,127}));
    connect(add.y, min.u2)
      annotation (Line(points={{-21,18},{-12,18},{-1,18}}, color={0,0,127}));
    connect(min.y, max2.u1) annotation (Line(points={{10.5,21},{14,21},{14,24},{16.8,
            24}}, color={0,0,127}));
    connect(min1.y, max1.u1) annotation (Line(points={{8.5,-47},{12,-47},{12,-44},
            {16.8,-44}}, color={0,0,127}));
    connect(add1.y, min1.u1)
      annotation (Line(points={{-19,-44},{-3,-44},{-3,-44}}, color={0,0,127}));
    connect(min1.u2, min.u1) annotation (Line(points={{-3,-50},{-8,-50},{-8,56},{-4,
            56},{-4,24},{-1,24}}, color={0,0,127}));
    connect(realExpression1.y, max1.u2) annotation (Line(points={{-19,-76},{10,-76},
            {10,-50},{16.8,-50}}, color={0,0,127}));
    connect(max2.u2, max1.u2) annotation (Line(points={{16.8,18},{14,18},{14,16},{
            10,16},{10,-50},{16.8,-50}}, color={0,0,127}));
    connect(max1.y, game.player2) annotation (Line(points={{30.6,-47},{40,-47},{40,
            -15},{50,-15}}, color={0,0,127}));
    connect(max2.y, game.player1) annotation (Line(points={{30.6,21},{40,21},{40,-3},
            {50,-3}}, color={0,0,127}));
...
*/
    Game game annotation (Placement(transformation(extent={{4,18},{32,38}})));
    Modelica.Blocks.Math.Gain gain(k=1/1032)
      annotation (Placement(transformation(extent={{-80,40},{-60,60}})));
    Modelica.Blocks.Math.Gain gain1(k=1/1032)
  ...
  equation
    ...
    connect(gain1.u, integerToReal1.y)
      annotation (Line(points={{-82,10},{-99,10}}, color={0,0,127}));
    connect(gain.y, game.player1) annotation (Line(points={{-59,50},{-28,50},{
            -28,35},{4,35}}, color={0,0,127}));
    connect(gain1.y, game.player2) annotation (Line(points={{-59,10},{-28,10},{
            -28,23},{4,23}}, color={0,0,127}));
    annotation (Diagram(coordinateSystem(extent={{-180,-100},{100,100}},
            preserveAspectRatio=false)),                                   Icon(
          coordinateSystem(extent={{-180,-100},{100,100}})));
  end Game_arduino;
  ...
  Modelica_DeviceDrivers.Blocks.OperatingSystem.SynchronizeRealtime
    ...
    annotation (Placement(transformation(extent={{-140,-40},{-120,-20}})));
*/
    Modelica.Blocks.Interfaces.RealInput player1 "from 0 to 1"
      annotation (Placement(transformation(extent={{-200,50},{-160,90}})));
    Modelica.Blocks.Interfaces.RealInput player2 "from 0 to 1"
